...
This command prints the logs for the container in the `etcd-controlplane` pod:

```shell
kubectl logs etcd-controlplane
```

Logs can have different severity levels.
For example, the output of the preceding command includes the following two lines:
...
This command runs `ps aux` to list the processes running in the first container of the `canal` pod:
...
When you clone a pod, specify the  `--copy-to` flag to name the new pod and the `--image` flag to specify the image to use for the debug container.
